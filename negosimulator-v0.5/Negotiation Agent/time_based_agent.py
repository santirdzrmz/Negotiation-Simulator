...
class TimeBasedAgent(Agent):

    def __init__(self, offer_space: OfferSpace, utility_function:UtilityFunction, reservation_value:float, agent_index:int, opponent_utility_model:OpponentUtilityModel, init_value:float, target_value:float, concession_param:float, agent_name="TimeBasedAgent"):
        ...
        self.opponent_model: OpponentUtilityModel = opponent_utility_model
        ...
        self.most_selfish_offer = None
        highest_util = 0
        for offer in self.offer_space.get_all_offers():
            
            util = self.utility_function.get_utility(offer)

            if self.most_selfish_offer == None or util > highest_util:
                self.most_selfish_offer = offer
                highest_util = util
    ...
    def choose_action(self, current_time:float, received_offer):
        ...
        approximate_opponent_utility = self.opponent_model.get_approximate_utility_function()

        ## BIDDING STRATEGY:

            ## 1. Calculate our aspiration level.
        exponent = 1 - (current_time / self.deadline)
        if(self.concession_param == 1):
            ...
            aspiration_level = (self.init_value - self.target_value) * (exponent)  + self.target_value
        else:
            aspiration_level = (self.init_value - self.target_value) * (1-pow(self.concession_param, exponent))/(1-self.concession_param)  + self.target_value
        ...
        if next_offer == None:
            next_offer = self.most_selfish_offer
